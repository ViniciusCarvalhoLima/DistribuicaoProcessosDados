...
class ClienteGrafico:
    ...
    def criar_interface(self):
        ...
        def centralizar_conteudo(evento):
            largura_canvas = evento.width
            largura_conteudo = 920
    ...
    def criar_area_sensores(self, frame_principal):
        ...
        botao_listar.pack(fill="x")

    def criar_area_comandos(self, frame_principal):
        frame = ttk.LabelFrame(
            frame_principal,
            text="Enviar comando para sensor",
            padding=15
        )
        frame.pack(fill="x", pady=8)

        ttk.Label(frame, text="ID do sensor:").grid(
            row=0,
            column=0,
            sticky="w",
            pady=5
        ...
        ttk.Label(frame, text="Comando:").grid(
            row=1,
            column=0,
            sticky="w",
            pady=5
    ...
    def criar_area_comandos_globais(self, frame_principal):
        ...
        botao_desligar_todos = ttk.Button(
            frame,
            text="Desligar todos os sensores",
            command=self.desligar_todos
        )
        botao_desligar_todos.grid(
            row=0,
            column=0,
            padx=5,
            sticky="ew"
        ...
        botao_ligar_todos = ttk.Button(
            frame,
            text="Ligar todos os sensores",
            command=self.ligar_todos
        )
        botao_ligar_todos.grid(
            row=0,
            column=1,
            padx=5,
            sticky="ew"
    ...
    def criar_area_consultas(self, frame_principal):
        ...
        self.entrada_sensor_consulta.pack(
            side="left",
            fill="x",
            expand=True
        )
        ...
        ttk.Button(
            linha_botoes,
            text="Média",
            command=self.consultar_media
        ).pack(side="left", fill="x", expand=True, padx=5)

        ttk.Button(
            linha_botoes,
            text="Desvio padrão",
            command=self.consultar_desvio
        ).pack(side="left", fill="x", expand=True, padx=5)

        ttk.Button(
            linha_botoes,
            text="Maior variação",
            command=self.consultar_maior_variacao
        ).pack(side="left", fill="x", expand=True, padx=5)
    ...
    def consultar_media(self):
        ...
        mensagem = f"MEDIA|{id_sensor}|{campo}"
        resposta = enviar_mensagem_gateway(mensagem)
    ...
    def consultar_desvio(self):
        ...
        mensagem = f"DESVIO|{id_sensor}|{campo}"
        resposta = enviar_mensagem_gateway(mensagem)
    ...
    def consultar_maior_variacao(self):
        ...
        mensagem = f"MAIOR_VARIACAO|{campo}"
        resposta = enviar_mensagem_gateway(mensagem)
